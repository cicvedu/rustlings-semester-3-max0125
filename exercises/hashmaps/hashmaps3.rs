...
fn build_scores_table(results: String) -> HashMap<String, Team> {
    ...
    for r in results.lines() {
        ...
        if team_1.is_some() {
            let x = team_1.unwrap();
            scores.insert(team_1_name, Team{ goals_scored: x.goals_scored + team_1_score, goals_conceded: x.goals_conceded + team_2_score });
        } else {
        ...
        if team_2.is_some() {
            let x = team_2.unwrap();
            scores.insert(team_2_name, Team{ goals_scored: x.goals_scored + team_2_score, goals_conceded: x.goals_conceded + team_1_score });
        } else {
